//
//  UIScrollView+QZKeyboardAvoidingAdditions.m
//  QZKeyboardAvoiding
//
...
@interface QZKeyboardAvoidingState : NSObject
...
@implementation UIScrollView (QZKeyboardAvoidingAdditions)

- (QZKeyboardAvoidingState*)keyboardAvoidingState {
    QZKeyboardAvoidingState *state = objc_getAssociatedObject(self, &kStateKey);
    if ( !state ) {
        state = [[QZKeyboardAvoidingState alloc] init];
...
- (void)QZKeyboardAvoiding_keyboardWillShow:(NSNotification*)notification {
    ...
    QZKeyboardAvoidingState *state = self.keyboardAvoidingState;
    
    UIView *firstResponder = [self QZKeyboardAvoiding_findFirstResponderBeneathView:self];
    ...
    if ( [self isKindOfClass:[QZKeyboardAvoidingScrollView class]] ) {
    ...
    self.contentInset = [self QZKeyboardAvoiding_contentInsetForKeyboard];
    
    CGFloat viewableHeight = self.bounds.size.height - self.contentInset.top - self.contentInset.bottom;
    [self setContentOffset:CGPointMake(self.contentOffset.x,
                                       [self QZKeyboardAvoiding_idealOffsetForView:firstResponder
...
- (BOOL)QZKeyboardAvoiding_focusNextTextField {
    ...
    UIView *view = [self QZKeyboardAvoiding_findNextInputViewAfterView:firstResponder beneathView:self];
...
-(void)QZKeyboardAvoiding_scrollToActiveTextField {
    ...
    CGPoint idealOffset = CGPointMake(0, [self QZKeyboardAvoiding_idealOffsetForView:[self QZKeyboardAvoiding_findFirstResponderBeneathView:self]
...
- (UIView*)QZKeyboardAvoiding_findFirstResponderBeneathView:(UIView*)view {
    ...
    for ( UIView *childView in view.subviews ) {
        ...
        UIView *result = [self QZKeyboardAvoiding_findFirstResponderBeneathView:childView];
        if ( result ) return result;
    }
    return nil;
}

- (UIView*)QZKeyboardAvoiding_findNextInputViewAfterView:(UIView*)priorView beneathView:(UIView*)view {
    UIView * candidate = nil;
    [self QZKeyboardAvoiding_findNextInputViewAfterView:priorView beneathView:view bestCandidate:&candidate];
    return candidate;
}

- (void)QZKeyboardAvoiding_findNextInputViewAfterView:(UIView*)priorView beneathView:(UIView*)view bestCandidate:(UIView**)bestCandidate {
    ...
    for ( UIView *childView in view.subviews ) {
        if ( [self QZKeyboardAvoiding_viewIsValidKeyViewCandidate:childView] ) {
            ...
            [self QZKeyboardAvoiding_findNextInputViewAfterView:priorView beneathView:childView bestCandidate:bestCandidate];
        }
    }
}

- (BOOL)QZKeyboardAvoiding_viewIsValidKeyViewCandidate:(UIView *)view {
...
- (void)QZKeyboardAvoiding_assignTextDelegateForViewsBeneathView:(UIView*)view {
    for ( UIView *childView in view.subviews ) {
        if ( ([childView isKindOfClass:[UITextField class]] || [childView isKindOfClass:[UITextView class]]) ) {
            [self QZKeyboardAvoiding_initializeView:childView];
        } else {
            [self QZKeyboardAvoiding_assignTextDelegateForViewsBeneathView:childView];
        }
    }
}
...
- (UIEdgeInsets)QZKeyboardAvoiding_contentInsetForKeyboard {
    QZKeyboardAvoidingState *state = self.keyboardAvoidingState;
...
-(CGFloat)QZKeyboardAvoiding_idealOffsetForView:(UIView *)view withViewingAreaHeight:(CGFloat)viewAreaHeight {
...
- (void)QZKeyboardAvoiding_initializeView:(UIView*)view {
    if ( [view isKindOfClass:[UITextField class]]
        ...
        UIView *otherView = [self QZKeyboardAvoiding_findNextInputViewAfterView:view beneathView:self];
...
@implementation QZKeyboardAvoidingState
@end
